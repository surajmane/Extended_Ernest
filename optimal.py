...
class ExperimentDesign(object):
    ...
    '''
    def __init__(self, parts_min, parts_max, total_parts, skew=0,
        ...
        :param skew: The skewness in the dataset
        :type skew: float
    ...
    def _construct_constraints(self, lambdas, points, skew):
        '''Construct non-negative lambdas and budget constraints'''
        constraints = []
        constraints.append(0 <= lambdas)
        constraints.append(lambdas <= 1)
        constraints.append(self._get_cost(lambdas, points, skew) <= self.budget)
        return constraints

    def _get_cost(self, lambdas, points, skew):
        ...
        for i in range(0, num_points):
            ...
            if skew == 0:
                ...
                cost = cost + (float(scale) / scale_min * 1.0 / float(mcs) * lambdas[i] * skew)
        return cost

    def _get_training_points(self):
        '''Enumerate all the training points given the params for experiment design'''
        mcs_range = range(self.mcs_min, self.mcs_max + 1)
...
def _get_features(training_point):
    ...
    skew = training_point[2]
    return [1.0, float(scale) / float(mcs), float(mcs), np.log(mcs), skew]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--skew', type=float, required=True,
        help='Total skew in the dataset')
    ...
    ex = ExperimentDesign(args.min_parts, args.max_parts, args.total_parts, args.skew,
    ...
    with open('opt_1op.csv', 'w', newline='') as file, open('compare_pr.csv', 'r') as file1:
        with open('compare_1pr.csv', 'w', newline='') as file2:
            writer = csv.writer(file)
            reader = csv.reader(file1)
            writer2 = csv.writer(file2)
            writer.writerow(["#Cores", "Input Fraction"])
            tryi = expts
            
            for expt in expts:
                writer.writerow([int(expt[2] * args.cores_per_mc), round(float(expt[1]),6)])    
                print ("%d, %d, %f, %d, %f" % (expt[2], expt[2] * args.cores_per_mc, expt[1], expt[0], expt[3]))
            
            for row in reader:
                if row[0][0] == '#':
                    row.append("#Input_Fraction_skew")
                    writer2.writerow(row)
                else:
                    for t in tryi:  
                        parts = row[0].split(',')
                        #print(int(parts[0])," ",int(t[2] * args.cores_per_mc))
                        if int(parts[0]) == int(t[2] * args.cores_per_mc):
                            row.append(round(float(t[1]),6))
                            writer2.writerow(row)
                            tryi.remove(t)
                            break
